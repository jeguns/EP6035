...
set.seed(315)
arima.sim(model=list(ma=c(theta)), n=150, sd = sigma) -> Y.MA1A 
arima.sim(list(order = c(0,0,1), ma = c(theta)), n = 150, sd = sigma) -> Y.MA1A # AR(1) con phi = 0.85
...
Y.IMA1A %>% 
  TSA::acf(type="correlation", lag = 50, 
           plot = TRUE, main = "Función de autocorrelación")

Y.IMA1A %>% 
  TSA::acf(type="partial", lag = 50, 
           plot = TRUE, main = "Función de autocorrelación parcial")

Y.IMA1A %>% diff %>% plot
Y.IMA1A %>% diff %>% aTSA::stationary.test(method = "kpss",lag.short=T)
Y.IMA1A %>% diff %>% aTSA::stationary.test(method = "kpss",lag.short=F)
Y.IMA1A %>% diff %>% aTSA::stationary.test(method = "adf")
Y.IMA1A %>% diff %>% aTSA::stationary.test(method = "pp",lag.short=F)
Y.IMA1A %>% diff %>% aTSA::stationary.test(method = "pp",lag.short=T)
Y.IMA1A %>% diff %>% BoxCox.lambda()
Y.IMA1A %>% diff %>% archTest()
Y.IMA1A %>% diff %>% McLeod.Li.test(y=.)
...
library(writexl)
writexl::write_xlsx(x    = data.frame(Y = Y.IMA1A), 
                    path = "Serie_simulada.xlsx")


# ARIMA(2,1,1) con phi1 = 0.7, phi2 = -0.18 y theta = 0.7 --------------------------------

arima.sim(list(order = c(2,1,1), 
               ar    = c(0.7,-0.18),
               ma    = c(-0.7)), 
          n = 150) -> serie2
